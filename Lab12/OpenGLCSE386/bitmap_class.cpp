...
bool CBitmap::loadBMP(const char *file_name)
{
	...
	// Read the Information file header.
	if(!fread(&bmp_infoheader, sizeof(BITMAPINFOHEADER), 1, bmp_file))
	{
		fclose(bmp_file);
		cerr << "Could not read file information header" << endl;
		return false;
	}
	...
	{
		fclose(bmp_file);
		cerr << "file is not 24 or 32 bit" << endl;
		return false;
	}
	...
	{
		fclose(bmp_file);
		cerr << "Error reading texture file" << endl;
			return false;
	}
	...
	{
		fclose(bmp_file);
		cerr << "Error finding the beginning of data in the texture file." << endl;
			return false;
	}
